 ...
 */
extern int NbMsg_forward[taille_forward]={0};
extern  uint16_t node_forward[taille_forward]={0};
struct listenport {
  uint16_t port;
  struct process *p;
};
    ...
    if(uip_ds6_is_addr_onlink(&UIP_IP_BUF->destipaddr)){
      ...
      if(route == NULL) {
	   //printf("src address %u \n",&UIP_IP_BUF->srcipaddr);
           for (i=0;i<taille_forward;i++){
                //printf("node id is %u\n",node_id);
                
                if(node_id !=UIP_IP_BUF->srcipaddr.u8[14]){
                                                             if(node_forward[i]==0 ){ 
                                                                    node_forward[i]=UIP_IP_BUF->srcipaddr.u8[14];
                                                                    NbMsg_forward[i]++;
                                                                    printf("I forward %d packet from %u \n",NbMsg_forward[i],node_forward[i]);
                                                                    break;
                                                                   }
                                                              if(node_forward[i] ==UIP_IP_BUF->srcipaddr.u8[14]){ 
                                                                    //node_forward[i]=&UIP_IP_BUF->srcipaddr;
                                                                    NbMsg_forward[i]++;
                                                                    printf("I forward %d packets from %u \n",NbMsg_forward[i],node_forward[i]);
                                                                    break;
                                                                   }
                                                            }
           }             
